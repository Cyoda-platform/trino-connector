...
public enum CyodaStaticReportTable {
    REPORTS(Arrays.asList(ReportsColumnDef.values())),
    REPORT_DETAILS(Arrays.asList(ReportDetailsColumnDef.values())),
    REPORT_STATS(StandardColumnDefinition.builder()
            .add(DistributedReportInfoView.meta())
            .build()),
    REPORT_HISTORIES(Arrays.asList(ReportHistoryColumnDef.values())),
    REPORT_GROUPS(StandardColumnDefinition.builder()
            ...
            .add(new StandardColumnDefinition(0, HISTORY_REPORT_NAME_VARIABLE, STRING))
    ...
    CyodaStaticReportTable(List<ColumnDefinition> columns) {
    ...
    public Optional<ColumnDefinition> getColumn(String name){
        return Optional.ofNullable(columns.get(name));
    }
    public @Nonnull ColumnDefinition getColumnNN(String name){
    ...
    public Map<AbstractTableHolder.TableDefinitionHandle, List<ColumnDefinition>> getFieldDefs() {
        return Collections.singletonMap(asTableDefinitionHandle(this.name()), ImmutableList.copyOf(this.getColumns()));
    }
