...
{
    PredicatePushdownController FULL_PUSHDOWN = (threshold, domain) -> {
        if (getDomainSize(domain) > threshold) {
    ...
    PredicatePushdownController CASE_INSENSITIVE_CHARACTER_PUSHDOWN = (threshold, domain) -> {
        checkArgument(
        ...
        if (domain.isOnlyNull()) {
            return FULL_PUSHDOWN.apply(threshold, domain);
        }

        if (!domain.getValues().isDiscreteSet()) {
            // case insensitive predicate pushdown could return incorrect results for operators like `!=`, `<` or `>`
            return DISABLE_PUSHDOWN.apply(threshold, domain);
        }

        Domain simplifiedDomain = domain.simplify(threshold);
    ...
    DomainPushdownResult apply(int threshold, Domain domain);
